( function ( global, factory ) {
	...
	function cottonAnimation( scope ) {
		...
		params.centerMouse
			? ( el.style.transform = 'translate(calc(-50% + '
					.concat( mouseData.x, 'px), calc(-50% + ' )
					.concat( mouseData.y, 'px))' ) )
			: ( el.style.transform = 'translate('
					.concat( mouseData.x, 'px, ' )
					.concat( mouseData.y, 'px)' ) );
	}
	function airModeAnimation( scope ) {
		...
		const originX =
			typeof mouseData.transformX === 'number'
				? mouseData.transformX + 'px'
				: mouseData.transformX;
		const originY =
			typeof mouseData.transformY === 'number'
				? mouseData.transformY + 'px'
				: mouseData.transformY;
		const transformX = mouseData.transformX
			? 'calc('
					.concat( originX, ' + ' )
					.concat( roundToTwo( airX / airMode.resistance ), 'px)' )
			: ''.concat( roundToTwo( airX / airMode.resistance ), 'px' );
		const transformY = mouseData.transformY
			? 'calc('
					.concat( originY, ' + ' )
					.concat( roundToTwo( airY / airMode.resistance ), 'px)' )
			: ''.concat( roundToTwo( airY / airMode.resistance ), 'px' );
		el.style.transform = 'translate('
			.concat( transformX, ', ' )
			.concat( transformY, ')' );
		...
		if ( init ) {
			...
			models.forEach( function ( item ) {
				item.removeEventListener(
					'mouseenter',
					scope.enterModelHandler
				);
				item.removeEventListener(
					'mouseleave',
					scope.leaveModelHandler
				);
			} );
	...
	const Cotton = /*#__PURE__*/ ( function () {
		function Cotton( element, options ) {
			...
			this.scene =
				this.params.scene instanceof Element
					? this.params.scene
					: document.querySelector( this.params.scene );
			this.models = NodeList.prototype.isPrototypeOf( this.params.models )
				? this.params.models
				: document.querySelectorAll( this.params.models );
			this.enterModelHandler = this.enterModelHandler.bind( this );
			this.leaveModelHandler = this.leaveModelHandler.bind( this );
			if ( ! this.element ) {
				return warn(
					'Cannot define a cotton element which is not exist'
				);
			}
			if ( ! this.scene ) {
				return warn( 'Cannot define a scene which is not exist' );
			}
			if ( this.params.speed > 1 || this.params.speed <= 0 ) {
				this.params.speed = 0.125;
			}
			...
			if ( this.params.airMode ) {
				...
				if (
					_typeof( airMode ) !== 'object' ||
					Array.isArray( airMode )
				) {
					this.params.airMode = airDefaults;
				} else {
					this.params.airMode = Object.assign( airDefaults, airMode );
				}
				if ( airMode.resistance < 1 || airMode.resistance > 100 ) {
					airMode.resistance = 15;
				}
			}
			...
			[
				{
					...
					value:
						...
						function enterModelHandler( e ) {
							...
							if (
								params.on.enterModel &&
								typeof params.on.enterModel === 'function'
							) {
								params.on.enterModel.call(
									this,
									el,
									e.target,
									e
				...
				},
				{
					...
					value: function leaveModelHandler( e ) {
						...
						const params = this.params;
						if (
							params.on.leaveModel &&
							typeof params.on.leaveModel === 'function'
						) {
							params.on.leaveModel.call( this, el, e.target, e );
						}
				...
				},
				{
					...
					value: function move() {
						...
						this.element.classList.add(
							this.params.conttonInitClass
						);
						if ( ! mouseData.animationFrame ) {
							airMode
								? airModeAnimation( this )
								: cottonAnimation( this );
						}
					}, // stop animation
				},
				{
			...
			[
				{
					...
					value: function getMouseData( scope ) {
						...
						scene.addEventListener( 'mousemove', function ( e ) {
							...
							if (
								_toConsumableArray( el.classList ).indexOf(
									params.conttonInitClass
								) > -1
							) {
								el.classList.add( params.cottonMovingClass );
							}
							if (
								params.on.cottonMove &&
								typeof params.on.cottonMove === 'function'
							) {
								params.on.cottonMove.call( scope, el, e );
							}
						} );
						...
						if ( airMode ) {
							...
							scene.addEventListener( 'mousemove', function () {
								...
								const maxX =
									window.innerWidth +
									mouseData.rect.width / 2;
								const maxY =
									window.innerHeight +
									mouseData.rect.height / 2;
								const distanceX =
									mouseData.mouseX - mouseData.rect.centerX;
								const distanceY =
									mouseData.mouseY - mouseData.rect.centerY;
								mouseData.distanceX = Math.min(
									Math.max( parseInt( distanceX ), -maxX ),
									maxX
								);
								mouseData.distanceY = Math.min(
									Math.max( parseInt( distanceY ), -maxY ),
									maxY
								);
							} );
						}
				...
				},
				{
					...
					value: function init( scope ) {
						...
						scene.addEventListener( 'mouseenter', function ( e ) {
							if (
								params.on.enterScene &&
								typeof params.on.enterScene === 'function'
							) {
								params.on.enterScene.call(
									scope,
									el,
									scene,
									e
						...
						scene.addEventListener( 'mouseleave', function ( e ) {
							...
							if (
								params.on.leaveScene &&
								typeof params.on.leaveScene === 'function'
							) {
								params.on.leaveScene.call(
									scope,
									el,
									scene,
									e
